...
| v3 部署配套 | `v3-deploy` | 腾讯云部署三方案 + Dockerfile/compose + 一键脚本 + 文档 |
| v4 联机体验 | `4fa2a8d` | 同机多窗口开局 bug 修复、托管策略房间设置、连接地址配置、AI 节奏、全屏施法特效 |
| v4.1 轮末重构 | `0b16c4c` | 手动下一轮（房主控制）、全员手牌复盘、牌堆剩余显示、结算面板不挡手牌 |
| v5 UI 美化 | `b7a0ea5` | 视觉模型评估驱动的对比度/层次/间距全面优化 |
...
- 本地 vs AI：首页选「本地 vs AI」，可调 AI 行动节奏。
- 联机（局域网）：同一网络下手机/其他电脑访问 `http://<你电脑的局域网IP>:5173`；
  加入方在联机大厅「服务器地址」留空即可（自动使用当前网址）。
- 联机（公网/腾讯云）：把前端部署后，加入方在「服务器地址」填 `http://<IP或域名>:端口`；
  同一台服务器上部署则留空自动。
...
# 同机双窗口联机回归（复现并锁定「无法开局」bug）：
node scripts/e2e-twowindow.mjs
```

## 断线 / 退出重进管理方案

- **身份识别**：每个座位 = (房间码, playerId)，playerId 存于本机 localStorage，按房间码分份存储。
- **普通加入**永远不带凭据（避免同机多窗口互相顶号）；只有显式点「重新加入上次的房间」才携带凭据恢复原座位。
- **托管策略**（房间设置，房主可调）：断线后 立即 / 等 15 秒 / 等 60 秒 由 AI 托管；
  等待期内界面显示「⏳ 断线等待重连」，到期转「🤖 AI 托管中」。
- **真人回来即接管**：AI 只是临时代理，凭据重连成功后立即恢复真人操作。
- **房主断线**：房主身份自动转移给其他在线真人，保证能继续开局/开始下一轮。
- **房间生命周期**：所有真人退出或断开后房间自动关闭回收。

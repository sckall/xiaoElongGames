...
- **音景**：脚步声按步幅触发 + 很轻的风声环境底噪；命中/受击/击杀各有独立音色。

## 3. 训练场验证方法

详情页选「🎯 训练场」（仅本地）：固定圆靶 / 移动圆靶 / 固定人靶 / 移动人靶，
HUD 实时显示命中率、命中发数、爆头率、总伤害与击碎数；圆靶 1 血即碎，
人靶 100 血。推荐先打固定靶调首发精度，再打移动靶调 bloom 恢复节奏。

## 4. 建议的调校顺序（配合 ?debug=1 + 移动测试 AI）

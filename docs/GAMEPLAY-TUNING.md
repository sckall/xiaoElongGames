...
| `client` | `mouseSensitivity` / `interpolationRate` / `interpolationBufferMs` / `correctionRate` / `softCorrectionThreshold` / `maxDeltaMs` / `maxPixelRatio` / `shadows` / `antialias` / `autoQuality` | 灵敏度、插值缓冲、服务端校正、回滚阈值、画质与自适应帧率 | ✅ |
...
> 训练场：详情页选「🎯 训练场」，HUD 实时显示命中率/命中发数/爆头率/总伤害，
> 适合调完数值后打固定/移动靶验证；散布拉杆配合动态准星即可肉眼观察 bloom。
